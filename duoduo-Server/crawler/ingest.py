...
def run_ingest():
    ...
    if all_docs:
        logger.info("開始 LLM 資料增強...")
        all_docs = enrich_documents(all_docs)
        add_documents(all_docs)
        pg_count = upsert_resources(all_docs)
        logger.info("資料攝取完成，ChromaDB %d 筆，PostgreSQL %d 筆", len(all_docs), pg_count)
    else:
